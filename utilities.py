...
def create_masks(inputs, targets):
    ...
    input_mask = tf.math.equal(inputs, 0)
    input_mask = input_mask[:, tf.newaxis, tf.newaxis, :]  # add extra dimensions for broadcasting

    # Create a padding mask for the targets
    target_mask = tf.math.equal(targets, 0)
    target_mask = target_mask[:, tf.newaxis, tf.newaxis, :]  # add extra dimensions for broadcasting

    # Create a look-ahead mask for the targets
    seq_len = tf.shape(targets)[1]
    look_ahead_mask = tf.linalg.band_part(tf.ones((seq_len, seq_len)), -1, 0)
    look_ahead_mask = 1 - look_ahead_mask  # invert the mask
    dec_target_padding_mask = tf.math.equal(targets, 0)
    look_ahead_mask = tf.maximum(look_ahead_mask, dec_target_padding_mask[:, tf.newaxis, tf.newaxis, :])  # broadcast to add extra dimensions

    # Combine the masks
    combined_mask = tf.maximum(input_mask, look_ahead_mask)
